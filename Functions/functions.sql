...
BEGIN
    ...
    IF @idade >= 18
        SET @r = 'MAIOR DE IDADE'
    ELSE
        SET @r = 'MENOR DE IDADE'
    RETURN @r
END
GO
...
BEGIN
    ...
    BEGIN
        ...
        SET @i = @i + 1
    END
    DECLARE @resto INT = @soma % 11
    DECLARE @d1 INT = CASE WHEN @resto < 2 THEN 0 ELSE 11 - @resto END
...
SELECT dbo.fn_Triplo(5)                    AS fn_Triplo          -- 15
SELECT dbo.fn_Quadrado(7)                  AS fn_Quadrado         -- 49
SELECT dbo.fn_ParImpar(4)                  AS fn_ParImpar         -- PAR
SELECT dbo.fn_Maioridade(20)               AS fn_Maioridade       -- MAIOR DE IDADE
SELECT dbo.fn_Maioridade1('2000-01-01')    AS fn_Maioridade1      -- MAIOR DE IDADE
SELECT dbo.fn_Desconto(200.00, 10)         AS fn_Desconto         -- 180.00
SELECT dbo.fn_FormataCPF('123.456.789-09') AS fn_FormataCPF       -- 123456789-09
SELECT dbo.fn_Conceito(8.5)               AS fn_Conceito          -- B
SELECT dbo.fn_StrZero(100.34, 9)           AS fn_StrZero          -- 000010034
SELECT dbo.fn_ValidaCPF('529.982.247-25')  AS fn_ValidaCPF        -- 1
